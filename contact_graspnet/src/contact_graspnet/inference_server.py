...
parser.add_argument('--K', default=[522.1910329546544, 0.0, 320.5, 0.0, 522.1910329546544, 240.5, 0.0, 0.0, 1.0], help='Flat Camera Matrix, pass as "[fx, 0, cx, 0, fy, cy, 0, 0 ,1]"')
...
if __name__ == "__main__":
    ...
    with MLSocket() as s:
        ...
        with conn:
            ...
            red_grasps_cam, scores, pred_points, gripper_openings = grasp_estimator.predict_grasps(sess, data, constant_offset=False, convert_cam_coords=True, forward_passes=1)
            conn.send(scores)

    # with MLSocket() as k:
    #     k.connect((address, REMOTE_PORT)) # Connect to the port and host
    #     conn<.send(data)
    # Save results
    # np.savez('results/predictions_{}'.format(os.path.basename(p.replace('png','npz').replace('npy','npz'))), 
    #           pred_grasps_cam=pred_grasps_cam, scores=scores, contact_pts=contact_pts)
